...
root = 'Your Own Address For Test'
output = 'test.h5' 
patchsize = 512
factor = 3
n_frame = 7
...
for i in range(1,total_count+1):
    for seq_num in range(num_clip):
        ...
        for k in range(n_frame):
            hr_temp=io.imread(root+'/' + str(i+20).rjust(3, '0') +'/hr'+'/GT' + str(k+seq_num+1).rjust(4, '0') +'.tif')
            lr_temp=io.imread(root+'/' + str(i+20).rjust(3, '0') +'/lr_step2'+'/' + str(k+seq_num+1).rjust(3, '0') +'.tif')
